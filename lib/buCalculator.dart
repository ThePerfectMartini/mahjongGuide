...
buCalculator(handInfo hand) {
  int bu = 20;

  int head = hand.getHeadNumber();
  List<int> firstNumbers = hand.getFirstNumbers();

  for (int i = 0; i < hand.huro.length; i++){
    int calculatedBu = 32;
    switch (hand.huro[i]){ // 형태 분류
      case 2:
        calculatedBu ~/= 4;
      case 3:
        calculatedBu ~/= 8;
    ...
    bool isNeedTile = firstNumbers[i] > 40 || firstNumbers[i] % 10 == 1 || firstNumbers[i] % 10 == 9;
    if (!isNeedTile){
      calculatedBu ~/= 2;
    }
    bu += calculatedBu;
  ...
  // Todo: 머리 특수 부수 추가
  if (head == 40 + hand.globalWind) bu += 2;
  if (head == 40 + hand.playerWind) bu += 2;
  if (head > 44) bu += 2; // 자풍, 판풍, 삼원 그리고 연풍패만 해당되게

  // Todo: 대기 부수 추가
  if (hand.machiType == '간짱' ||
      hand.machiType == '변짱' ||
      hand.machiType == '단기') bu += 2;

  // Todo: 화료 타입별 부수 추가 - 순서상 마지막으로 옮기기
  bool pingfuTsumo = hand.getMenzenState() && hand.agariType && bu == 20;

  if (!pingfuTsumo && hand.agariType){
    bu += 2;
  }else if (hand.getMenzenState()) {
    bu += 10;
  }
